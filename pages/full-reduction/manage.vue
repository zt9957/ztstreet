    ...
    <el-dialog class="dialog" :title="formTitle" :close-on-click-modal="false" :visible.sync="formVisible">
        <el-form ref="dataForm" :rules="rules" :model="formData" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
            <el-form-item label="活动名称" prop="title">
                <el-input v-model="formData.title" clearable maxlength="20" placeholder="请输入活动名称" />
            </el-form-item>
			...
			<el-form-item label="减免金额" prop="coupon_money">
			    <el-input v-model="formData.coupon_money" clearable maxlength="4" placeholder="请输入减免金额" />
			</el-form-item>
...
<script>
    export default {
        data() {
            return {
                ...
                rules: {
					...
					coupon_money: [{required: true, message: '请输入减免金额', trigger: 'blur'}],
		...
		computed: {
			formTitle(){
				return this.formData._id ? '修改满减' : '添加满减';
			}
		},
        watch: {
			formVisible(state){
        ...
        methods: {
            submit(){
                this.$refs.dataForm.validate(async res=>{
					...
					const response = await this.$request('fullReduction', operation, data);
